...
app.post("/todos", authMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool.query("INSERT INTO todos (title, note, user_id) VALUES ($1, $2, $3) RETURNING *", [title, notes, user_id]);
...
app.post("/subtodos", authMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool.query("INSERT INTO subtodos (title, main_todo_id, user_id) VALUES ($1, $2, $3) RETURNING *", [title, main_todo_id, user_id]);
        return res.status(201).json(result.rows[0]);
    }
    catch (err) {
        console.log(err);
...
app.get("/subtodos", authMiddleware, async (req, res) => {
    try {
        const user_id = req.user.id;
        const result = await pool.query("SELECT * FROM subtodos WHERE user_id = $1", [user_id]);
        return res.status(200).json(result.rows);
    }
    catch (err) {
        console.log(err);
